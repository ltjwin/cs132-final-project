        ...
        for (var k = j; parseInt(k) <= parseInt(finish); k++) {
            ...
            for (i = 0; i < 32; i++) {
		         ...
		         cell1.style.backgroundColor = "red";
                         ...
                         cell1.style.backgroundColor = "#0717FA";
                     }
                     daycount++;
                }
   	    }
...
$(document).ready(function() {
      ...
      function sendQuery() {
	    ...
	    if (tableCreated == 1) {
            ...
            if(new Date(startDate)<min || new Date(startDate)>max) {
              alert("Please enter a start date between 2001-01-01 and 2012-12-31");
              return;
            }

            if(new Date(endDate)<min || new Date(endDate)>max) {
              alert("Please enter a valid end date between 2001-01-01 and 2012-12-31");
              return;
            }
      ...
      function queryDB(warningType, countyCodeCookie, startDate, endDate, callback){
              ...
              csvArray=[["Date", "Warning", "Warning Text"]];  
                ...
                if (request.status == 200){
                  ...
                  for (i = 0; i < data.length; i++) {
                    ...
                    //var text2 = document.createTextNode(warning);
                    ...
                    //console.log("Before textSpan: "+text);
                    //textSpan.innerHTML = text;
                    //textSpan.style.visibility = "hidden";
                    //button.appendChild(textSpan);
                    //td3.innerHTML = "<button>Show Source Text</button>";
                    
                    //button.onclick=function(){alert(text)};
                    //console.log("button's 2nd child's text: "+button.childNodes[1].innerText);
                    //button.onclick=function(){this.childNodes[1].innerText};
                    
                    //console.log("from storage: " + localStorage.getItem(button.id));

                    button.onclick=function(){console.log("this id: "+ this.id); sweetAlert(localStorage.getItem(this.id))};
      ...
      function download() {
        downloadCSV({filename: csvArray.csv});
      } 
